...
if __name__ == '__main__':
    ...
    model = Advrtset(dataset_X1.shape[1], ker_size=2, stride=1, dial=1) # .to(device)
    model2 = Advrtset(dataset_X2.shape[1], ker_size=2, stride=1, dial=1) # .to(device)
    parallel_net_1 = nn.DataParallel(model, device_ids=device_ids)
    parallel_net_2 = nn.DataParallel(model2, device_ids=device_ids)
    optimizer_1 = torch.optim.Adam(parallel_net_1.parameters(), lr=lr, weight_decay=1) # .to(device)  # , momentum=0.9)
    optimizer_2 = torch.optim.Adam(parallel_net_2.parameters(), lr=lr, weight_decay=1) #, momentum=0.9)

    parallel_net_1.train()  # put in training mode
    parallel_net_2.train()


    trainloader1 = torch.utils.data.DataLoader(
        dataset_1, batch_size=batch_size, shuffle=False, pin_memory=True)  # , num_workers=2)
    trainloader2 = torch.utils.data.DataLoader(
        dataset_2, batch_size=batch_size, shuffle=False, pin_memory=True)  # , num_workers=2)
    for epoch in range(1, num_epochs + 1):
        ...
        for i, data in enumerate(tqdm(zip(trainloader1, trainloader2), total=len(trainloader1)), 0):
            ...
            outputs1 = parallel_net_1(inputs1)  # forward pass
            outputs2 = parallel_net_2(inputs2)
            loss = cosine_loss(outputs1, outputs2, labels1, labels2, flag=2, lmbda=10, b=0)
    ...
    # infer
    parallel_net_1.eval()  # put in training mode
    parallel_net_2.eval()
